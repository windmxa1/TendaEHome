...
var getJsonArrayByPageSize = function(pageSize, pageNo) {
	$.ajax({
		type: "post",
		url: "back/repair/getRepairOrderList",
		dataType: "json",
		async: false,
		cache: false,
		headers: {
...
var builderUQTQueryMsg = function(UQTQueryMsg) {
	...
	var th = '<tr><th scope="col"  class="dis_order">预约时间</th><th scope="col" class="match_type" >内容</th><th scope="col" class="match_type">联系方式</th><th scope="col"  class="match_type">地点</th><th scope="col" class="query_pro">状态</th><th scope="col" class="dis_dta">操作</th><th class="dis_hidden" style="display: none">隐藏属性</th></tr>';
	...
	$.each(UQTQueryMsg, function(i, eachData) {
		tr = $('<tr>');
		var id = eachData.id;
		var appointmentTime = eachData.appointmentTime;
		var description = eachData.description;
		var phone = eachData.phone;
		var address = eachData.address;
		var state = eachData.state;
		tr.append("<td class='dis_order'>"+appointmentTime+"</td>"+
				"<td class='match_type'>"+description+"</td>"+
				"<td class='match_type'>"+phone+"</td>"+
				"<td class='match_type'>"+address+"</td>"+
				"<td class='query_pro'>"+state+"</td>"+
				"<td class='dis_dta'><a href='javascript:;' data-toggle='modal' data-target='#modOrder' class='editOp modOrder' data-order='"+JSON.stringify(eachData)+"'>修改</a><a href='javascript:;' class='editOp deleteOrder' data-oid='"+id+"'>删除</a></td>"
		);
...
//修改维修单
$(document).on('click','.modOrder',function(){
...
$(document).on('click','.deleteOrder',function(){
	...
	if(confirm('确定要删除这条维修单吗')){
		$.ajax({
		type: "post",
		url: "back/repair/deleteRepairOrder",
		data: {'id':oId},
		async: false,
		cache: false,
		headers: {
