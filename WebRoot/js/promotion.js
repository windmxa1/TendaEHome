...
var getJsonArrayByPageSize = function(pageSize, pageNo) {
	$.ajax({
		type: "post",
		url: "back/staff/getStaffPromotionList",
		dataType: "json",
		async: false,
		cache: false,
		headers: {
...
var builderUQTQueryMsg = function(UQTQueryMsg) {
	...
	var th = '<tr><th scope="col"  class="dis_order">地址</th><th scope="col" class="match_type" >员工号</th><th scope="col" class="match_type">推广时间</th><th scope="col"  class="match_type">员工姓名</th><th class="dis_hidden" style="display: none">隐藏属性</th></tr>';
	...
	$.each(UQTQueryMsg, function(i, eachData) {
		...
		var time = new Date(eachData.time).toLocaleString().replace(/:\d{1,2}$/,' ');   
		var staffName = eachData.staffName;
		tr.append("<td class='dis_order'>"+address+"</td>"+
				"<td class='match_type'>"+staffNo+"</td>"+
				"<td class='match_type'>"+time+"</td>"+
				"<td class='match_type'>"+staffName+"</td>");
