...
var getJsonArrayByPageSize = function(pageSize, pageNo) {
	...
	switch (fun_index) {
	case 0:
		url_path = "back/user/getUserList";
		user_data = {
		...
		break;
	case 1:
		url_path = "back/user/getUserByPhone";
		user_data = {
			'start' : (pageNo - 1) * pageSize,
			'limit' : pageSize,
		...
		break;
	case 2:
		url_path = "back/user/getFreeList";
		user_data = {
			'start' : (pageNo - 1) * pageSize,
			'limit' : pageSize,
		};
...
var builderUQTQueryMsg = function(UQTQueryMsg) {
	...
	var th = '<tr><th scope="col" class="eng_name" style="width:100px">昵称</th><th scope="col" class="query_pro" >联系电话</th><th class="match_type" scope="col">地址</th><th scope="col"  class="match_type">头像</th><th scope="col"  class="match_type">注册时间</th><th scope="col"  class="dis_order">操作</th><th class="dis_hidden" style="display: none">隐藏属性</th></tr>';
	...
	$.each(UQTQueryMsg,function(i, eachData) {
						...
						var txUrl = eachData.url;
						var createTime = eachData.createTime;
						var trString = "<td class='eng_name'>" + nickname
								+ "</td>" + "<td class='query_pro'>" + phone
								+ "</td>" + "<td class='match_type'>" + address
								+ "</td>" + "<td class='match_type' title="
								+ txUrl + ">" + txUrl + "</td>"
								+ "<td class='dis_order'>" + createTime
								+ "</td>" + "<td class='dis_order'>";
						if (eachData.isFree == 0) {
							trString = trString
									+ "<button class='btn btn-info' style='height:30px; margin-right:10px' onclick='setFree("
									+ id + "," + 1 + ")' >免单授权</button>";
						} else {
							trString = trString
									+ "<button class='btn btn-warning' style='height:30px; margin-right:10px' onclick='setFree("
									+ id + "," + 0 + ")' >取消免单授权</button>";
						}
						trString = trString
								+ "</td><td class='dis_hidden' style='display: none'></td>";
						tr.append(trString);
